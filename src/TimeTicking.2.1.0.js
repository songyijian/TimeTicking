...
!function(){
	function TimeTicking(obj){
		if(!obj.stopdata){
			console.error(' stopdata Ã»ÓÐÌîÐ´')
			return false;
		};
		this.data={
			type:obj.type || 'date',
			stopdata:obj.stopdata,		//"Jun 23, 2017 00:00:01"
			setIntervalData:  obj.setIntervalData && typeof obj.setIntervalData === "number" ? obj.setIntervalData : false,	//²»Ð¡ÓÚ1000£¨1Ãë£©
	...
	TimeTicking.prototype.init=function(){
		...
		if(this.data.type ==='long'){
			if(typeof this.data.stopdata !== 'number'){
				console.error('stopdata ²»ÊÇnumber ÀàÐÍ  ' )
				return;
			}
